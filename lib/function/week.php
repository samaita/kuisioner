...
$dateentry = $year.'-'.$month.'-'.$dayis;
$dateentry2 = date("Y-m-d");
...
$sql = "SELECT `nama`, `nomor_kontak`, `kritik_saran`, `nilai_plasa`, `pasang_indihome`, `entry_datetime` FROM `user_entries` WHERE blocked = 0 AND entry_datetime >='". $dateentry."' AND entry_datetime <= '".$dateentry2."'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        $kontak = strval($row["nomor_kontak"]);
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Laporan Kuisioner '.date("Y-m-d"));
